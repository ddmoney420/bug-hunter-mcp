...
import { CONCEPT_GRAPH, generateGraphVisualization, recommendNextConcept, generateMasteryReport, getConceptNode, listAllConcepts, } from "./graph.js";
...
async function analyzeRepo(params) {
    ...
    try {
        ...
        try {
            const tree = execSync(`find ${repoDir} -maxdepth 2 -type f -o -type d | grep -v '.git' | head -50`, { encoding: "utf-8" });
        ...
        if (issue_number) {
            try {
                ...
                result += `Labels: ${issue.data.labels.map((l) => l.name).join(", ")}\n`;
...
async function generateChantSpec(params) {
    ...
    try {
        ...
        const chantLabels = labels
            .filter((l) => !["good first issue", "help wanted", "bug", "enhancement"].includes(l.toLowerCase()))
            .map((l) => l.toLowerCase().replace(/\s+/g, "-"));
        // Determine spec type based on labels
        let specType = "code";
        if (labels.some((l) => l.toLowerCase().includes("doc"))) {
            specType = "task";
        }
...
async function showGraph(params) {
    ...
    if (!filter) {
        ...
        const filtered = listAllConcepts().filter((concept) => {
            return (concept.name.toLowerCase().includes(lowerFilter) ||
...
async function nextBug(params) {
    ...
    if (show_report) {
        ...
        const masteryStatuses = mastered_concepts.map((id) => ({
            conceptId: id,
            mastered: true,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        switch (name) {
            case "hunt_issues":
                result = await huntIssues(args);
                break;
            case "analyze_repo":
                result = await analyzeRepo(args);
                break;
            case "scaffold_solution":
                result = await scaffoldSolution(args);
                break;
            case "claim_issue":
                result = await claimIssue(args);
                break;
            ...
            case "get_strategy":
                ...
                break;
            case "show_graph":
                result = await showGraph(args);
                break;
            case "next_bug":
                result = await nextBug(args);
                break;
            default:
